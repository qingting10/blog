...
//    查全部
    ...
    List<ReplyUserVO> querySecondReplyByPidAndBid(@Param("parentId")int parentId, @Param("blogId")int blogId);

    //根据id  删除评论以及它的子评论（parendId==id）
    int deleteReplyByidAndPid(@Param("id")int id, @Param("parentId")int parentId);
