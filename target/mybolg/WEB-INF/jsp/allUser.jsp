...
<%--            <a class="btn btn-primary" href="${pageContext.request.contextPath}/user/toAddUser">新增</a>--%>
        ...
        <div class="col-md-8 column">
            <form class="form-inline" action="${pageContext.request.contextPath}/user/queryUser" method="post" style="float: right">
                ...
                <input type="text" name="username" class="form-control" placeholder="请输入要查询的用户名">
...
<%--                    <th>头像</th>--%>
                ...
                <tbody>
                <c:forEach var="users" items="${requestScope.get('list')}">
                    <tr>
                        <td>${users.getId()}</td>
...
<%--                        <td>${users.getAvatar()}</td>--%>
                        ...
                        <th>${users.getBlognum()}</th>
                        <th>${users.getFans()}</th>
                        <th>${users.getAppreciate()}</th>
                        <th>${users.getCommunicate()}</th>
                        <td>${users.getVisitedNumber()}</td>
                        <td>${users.getCreateTime()}</td>
                        <td>
                            <a class="red" href="${pageContext.request.contextPath}/user/del/${users.getId()}">注销</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
